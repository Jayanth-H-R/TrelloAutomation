...
public class TrelloEndToEndTest extends BaseClass {
	...
	public void createAndDeleteBoard() throws IOException, InterruptedException {
		try {
			...
			Thread.sleep(5000);
			actions.scrollToElement(userCreatedBoardPage.getCloseBoardOption()).pause(3000).perform();
			userCreatedBoardPage.getCloseBoardOption().click();
			userCreatedBoardPage.getCloseButton().click();
			webDriverUtils.elementToBeClickable(driver, userCreatedBoardPage.getPermanentDeleteBoard()).click();
			webDriverUtils.elementToBeClickable(driver, userCreatedBoardPage.getDeleteButton()).click();
			System.out.println("Board Successfully Deleted");
		} catch (SocketException e) {
			System.out.println("exception occur SocketException");
		}
	}
	...
	public void addListsToBoard() throws IOException, InterruptedException {
		try {
			...
			Actions actions = new Actions(driver);
			TrelloBoardPage boardPage = new TrelloBoardPage(driver);
			webDriverUtils.elementIsVisible(driver, boardPage.getCreateBoardNewOption()).click();
			//boardPage.getCreateBoardNewOption().click();
			boardPage.getBoardTitleTextfield().sendKeys(fileUtils.toReadData("boardtitle"));
			actions.moveToElement(boardPage.getCreateBoardbutton()).pause(1000)
					.clickAndHold(boardPage.getCreateBoardbutton()).build().perform();
			...
			if (userCreatedBoardsPage.getJiraPopupEle().isDisplayed()) {
				userCreatedBoardsPage.getMinimizePopupButton().click();
			}
			...
			actions.click(userCreatedBoardsPage.getAddThisList()).pause(2000).perform();
			actions.click(userCreatedBoardsPage.getExceptionsAddCard()).pause(1000)
					.sendKeys(fileUtils.toReadData("card1")).sendKeys(Keys.ENTER).build().perform();
			...
			webDriverUtils.elementToBeClickable(driver, userCreatedBoardsPage.getShowMenuOption());
			userCreatedBoardsPage.getShowMenuOption().click();
			// userCreatedBoardPage.getMoreOption().click();
			Thread.sleep(5000);
			actions.scrollToElement(userCreatedBoardsPage.getCloseBoardOption()).pause(3000).perform();
			userCreatedBoardsPage.getCloseBoardOption().click();
			userCreatedBoardsPage.getCloseButton().click();
			Thread.sleep(3000);
			webDriverUtils.elementToBeClickable(driver, userCreatedBoardsPage.getPermanentDeleteBoard()).click();
			webDriverUtils.elementToBeClickable(driver, userCreatedBoardsPage.getDeleteButton()).click();
			System.out.println("Board Successfully Deleted");

		} catch (SocketException e) {
			System.out.println("socket exception handled");
		}
	}
